...
class Guild(Cog):
  ...
  async def hacker(self, guild):
    ...
    embed = discord.Embed(
      title=f"{guild.name}'s Information", color=0x2f3136).set_author(
        name="Guild Joined",
        icon_url=guild.me.display_avatar.url if guild.icon is None else
        guild.icon.url).set_footer(text=f"{guild.name}",
                                   icon_url=guild.me.display_avatar.url
                                   if guild.icon is None else guild.icon.url)
    embed.add_field(
      name="**__About__**",
      value=
...
Threads : {len(guild.threads)}
    ...
    await me.send(f"{rope[0]}" if rope else "No Pre-Made Invite Found",
                  embed=embed)
    if not guild.chunked:
      await guild.chunk()
    embed = discord.Embed(
      title="\U0001f44b Hey, I am Aloa!",
      description=
      "Hello, thank you for adding me to your server. Here are some commands to get you started.",
      color=0x2f3136,
    )
    ...
    embed.add_field(
      name="vote",
      value="You can support Aloa by voting! Thank you!",
      inline=False,
    )
