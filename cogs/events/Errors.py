...
from core import Aloa, Cog, Context
    ...
    if isinstance(error, commands.CommandNotFound):
      ...
      if str(ctx.author.id) in data["ids"]:
        embed = discord.Embed(
          ...
          description=
          "You Are Blacklisted From Using My Commands.\nIf You Think That It Is A Mistake, You Can Appeal In Our Support Server By Clicking [here](https://dsc.gg/aloa-support)",
          color=0x2f3136)
      ...
      hacker = discord.Embed(
        color=0x2f3136,
        description=f"You Can\'t Use My Commands In Dm(s)",
      ...
      hacker = discord.Embed(
        color=0x2f3136,
        description=
        f"<:Aloa_claim:1096426195932954766> | {ctx.author.name} is on cooldown retry after {error.retry_after:.2f} second(s)",
        timestamp=ctx.message.created_at)
      ...
      hacker = discord.Embed(
        color=0x2f3136,
        description=
        f"<:Aloa_claim:1096426195932954766> | This Command is already going on, let it finish and retry after",
        timestamp=ctx.message.created_at)
      hacker.set_author(name=f"{ctx.author.name}",
                        icon_url=f"{ctx.author.avatar}")
      hacker.set_thumbnail(url=f"{ctx.author.avatar}")
      ...
      hacker = discord.Embed(
        color=0x2f3136,
        description=
        f"<:Aloa_claim:1096426195932954766> | You lack `{fmt}` permission(s) to run `{ctx.command.name}` command!",
        timestamp=ctx.message.created_at)
      hacker.set_author(name=f"{ctx.author.name}",
                        icon_url=f"{ctx.author.avatar}")
      hacker.set_thumbnail(url=f"{ctx.author.avatar}")
      ...
      await ctx.send(
        f'| I need the **{missing}** to run the **{ctx.command.name}** command!',
        delete_after=10)
