...
cluster = motor.motor_asyncio.AsyncIOMotorClient(
  "mongodb+srv://nafay123456:afay123456@cluster0.nfc6fim.mongodb.net/?retryWrites=true&w=majority"
)
...
class Utility(commands.Cog):
  ...
  def __init__(self, bot):
    ...
    self.db = self.connection["Aloa"]["servers"]
  ...
  async def stats(self, ctx):
    """Shows some usefull information about Aloa"""
    ...
    embed = discord.Embed(
      color=0x2f3136,
      description=
      "[Invite](https://discord.com/api/oauth2/authorize?client_id=1095639790709375016&permissions=8&scope=bot%20applications.commands) ● [Support Server](https://discord.gg/e5nseazsEJ)"
    )

    embed.add_field(name='<a:Discord:1085203728841310373> • **Servers**',
                    value=f'```Total: {serverCount} Server```')
    embed.add_field(name='<:user:1085203633622237184> • **Users**',
                    value=f'```Total: {total_members} Users```')
    embed.add_field(
      name="<:cpu:1077842016462970881> • **System**",
      value=
      f"```RAM: {used_memory}/{total_memory} MB\nCPU: {cpu_used}% used.```")
    embed.add_field(name="<:python:1096304203086966845>  • **Python Version**",
                    value=f"```{sys.version}```"),
    embed.add_field(
      name='<:python:1096304203086966845> • **Discord.py Version**',
      value=f'```{discord.__version__}```')
    embed.add_field(name="<a:ping:1096304331772411906> • **Ping**",
                    value=f"```{round(self.bot.latency * 1000, 2)} ms```")
    SIVAY = await self.bot.fetch_user(735003878424313908)

    embed.add_field(
      name='<:owner:1096302983077167174> • **Owner**',
      value=f""
      f"[~ Victorious Clasher#1430](https://discord.com/users/866577524917534720)\n"
    )
    embed.add_field(
      name='<:emoji_11:1060789858446098442> • **Co-Founder**',
      value=f"[Abhinay🥂#1818](https://discord.com/users/986142042025504789)\n"
      f"")
    embed.add_field(
      name='<:cx_flash_features:1095641230731382794> • **Supporters**',
      value=f"[Nafay#0001](https://discord.com/users/101073633107326017657)\n"
      f"")

    embed.set_author(name=f"{self.bot.user.name} Stats",
                     icon_url=self.bot.user.display_avatar.url)
    embed.set_thumbnail(url=self.bot.user.display_avatar.url)
    embed.set_footer(text='Thanks For Using Aloa',
                     icon_url=self.bot.user.display_avatar.url)
  ...
  async def invite(self, ctx: commands.Context):
    embed = discord.Embed(
      description=
      "> • [Click Here To Invite A To Your Server](https://discord.com/api/oauth2/authorize?client_id=1095639790709375016&permissions=8&scope=bot%20applications.commands)\n> • [Click Here To Join My Support Server](https://discord.gg/e5nseazsEJ)",
      color=0x2f3136)
...
**Discord.py Version:** {discord.__version__}
  ...
  async def serverinfo(self, ctx: commands.Context):
    ...
    embed.add_field(
      name="**__About__**",
      value=
      f"**Name : ** {guild.name}\n**ID :** {guild.id}\n**Owner <:owner:1096302983077167174> :** {guild.owner} (<@{guild.owner_id}>)\n**Created At : ** <t:{c_at}:F>\n**Members :** {len(guild.members)}",
      inline=False)
    ...
    if guild.features:
      embed.add_field(
        name="**__Features__**",
        value="\n".join([
          f"<:tick:1096303195480932372> : {feature.replace('_',' ').title()}"
...
**Total Emoji/Stickers :** {total_emojis}
    ...
    embed.add_field(
      ...
      value=
      f"Level : {guild.premium_tier} [<a:booster:1077680843620880495> {guild.premium_subscription_count} Boosts ]\nBooster Role : <@&{guild.premium_subscriber_role.id}>",
      inline=False)
  ...
  async def _userinfo(self,
    ...
    if member.public_flags.hypesquad:
      badges += "<:hypesquadevents:1096313745426350141> "
    if member.public_flags.hypesquad_balance:
      badges += "<:balance:1096313885251874836> "
    if member.public_flags.hypesquad_bravery:
      badges += "<:Hypesquad_Bravery:1096314041724588032> "
    if member.public_flags.hypesquad_brilliance:
      badges += "<:hypesquad:1096314180413444127> "
    if member.public_flags.early_supporter:
      badges += "<:earlysupporter:1096314300722860132> "
    if member.public_flags.active_developer:
      badges += "<:activedeveloper:1096314405215555644> "
    if member.public_flags.verified_bot_developer:
      badges += "<a:developer:1096304531442237480> "
    if member.public_flags.discord_certified_moderator:
      badges += "<a:staff:1096314682601635890> "
    if member.public_flags.staff:
      badges += "<a:staff:1096314682601635890> "
    if member.public_flags.partner:
      badges += "<a:partnered:1096314807679991888> "
...
**ID:** {member.id}
**Nickname:** {nickk}
**Bot?:** {'<:tick:1076042204310679562> Yes' if member.bot else '<:cross:1077478135794245743> No'}
**Badges:** {badges}
...
**Color:** {member.color if member.color else '000000'}
  ...
  async def removeemoji(self, ctx, emoji: discord.Emoji):
    await emoji.delete()
    await ctx.send("**<:tick:1076042204310679562> emoji has been deleted.**")
  ...
  async def unbanall(self, ctx):
    button = Button(label="Yes",
                    ...
                    emoji="<:tick:1076042204310679562>")
    button1 = Button(label="No",
                     style=discord.ButtonStyle.red,
                     emoji="<:cross:1077478135794245743>")
  ...
  async def _badges(self, ctx, user: Optional[discord.User] = None):
    ...
    if mem.public_flags.hypesquad:
      ...
      badges += "<:balance:1077670823311179826> **HypeSquad Balance**\n"

    elif mem.public_flags.hypesquad_bravery:
      badges += "<:hypesquadevents:1096313745426350141> **HypeSquad Bravery**\n"
    elif mem.public_flags.hypesquad_brilliance:
      badges += "<:hypesquad:1096314180413444127> **Hypesquad Brilliance**\n"
    if mem.public_flags.early_supporter:
      badges += "<:earlysupporter:1077672678372163655>**Early Supporter**\n"
    elif mem.public_flags.verified_bot_developer:
      badges += "<a:developer:1096304531442237480>  **Verified Bot Developer**\n"
    elif mem.public_flags.active_developer:
      badges += "<:activedeveloper:1096314405215555644> **Active Developer**\n"
    if badges == "":
      badges = "None"
    if bdgs == []:
      ...
      embed2.add_field(name="**Bot Badges:**",
                       value="<:cross:1077478135794245743> NO BADGES",
                       inline=False)
      embed2.set_author(
        name=f"{mem}",
  ...
  async def ping(self, ctx):
    embed = discord.Embed(
      title="""<:ping:1077667853517475850> Ping <:ping:1077667853517475850>""",
