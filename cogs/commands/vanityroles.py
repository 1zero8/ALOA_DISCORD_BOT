...
class Vanityroles(commands.Cog):
      ...
      if ctx.author == ctx.guild.owner or ctx.guild.me.top_role <= ctx.author.top_role:
        if role.permissions.administrator or role.permissions.ban_members or role.permissions.kick_members:
          embed1 = discord.Embed(
            description=
            "❌ | Please Select Role That Dont Have Any Type Of Dangerous Permissions.",
            color=0x2f3136)
          ...
          embed.set_author(name=f"Vanity Roles Config For {ctx.guild.name}",
    ...
    with open("vanityroles.json", "r") as f:
      ...
      if ctx.author == ctx.guild.owner or ctx.guild.me.top_role <= ctx.author.top_role:
        if str(ctx.guild.id) not in jnl:
          embed1 = discord.Embed(
            description=
            "❌ | This server don't have any vanity roles setupped yet .",
            color=0x2f3136)
          await ctx.reply(embed=embed1, mention_author=False)
        else:
          jnl.pop(str(ctx.guild.id))
          await ctx.reply(f"Vanity Role System Removed For This Guild.",
  ...
  async def config(self, ctx):
    ...
    if str(ctx.guild.id) not in jnl:
      ...
      embed.add_field(name=" Vanity", value=f"{vanity}", inline=False)
      embed.add_field(name=" Role", value=f"{randirole.mention}", inline=False)
      embed.add_field(name=" Channel",
