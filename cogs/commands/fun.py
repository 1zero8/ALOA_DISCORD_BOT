...
class Fun(commands.Cog):
  ...
  async def add_role(self, *, role: int, member: discord.Member):
    if member.guild.me.guild_permissions.manage_roles:
      ...
      await member.add_roles(role, reason="Cyber | Role Added ")

  async def remove_role(self, *, role: int, member: discord.Member):
    if member.guild.me.guild_permissions.manage_roles:
      role = discord.Object(id=int(role))
      await member.remove_roles(role, reason="Cyber | Role Removed")
      ...
      async with a.get(f"http://ipwhois.app/json/{ip}") as b:
        ...
        try:
          ...
          embed = discord.Embed(
            ...
            color=0x2f3136)
          embed.set_footer(
            text='Thanks For Using Cyber',
            icon_url=
            "https://images-ext-2.discordapp.net/external/9u3nFXP1ParBCvoKHBC5HBXB09b-y77pyZq548htxWA/%3Fsize%3D1024/https/cdn.discordapp.com/avatars/999564751317237760/28a9708bf9ed2e4e5564ccb3ec7ab215.png?width=670&height=670<a:crown1:1076528463370006578>"
          )
          await ctx.send(embed=embed)
        except KeyError:
          embed = discord.Embed(
...
############################
  ...
  async def _owner(self, context: Context, member: discord.Member) -> None:
    if data := getConfig(context.guild.id):
      ...
      hacker = discord.Embed(
        description=
        f"<a:botyes:1074767345517596713> | Successfully Given <@&{role}> To {member.mention}",
        color=0x2f3136)
      ...
      hacker1 = discord.Embed(
        description=f"❗ | Owner role is not setuped in {context.guild.name}",
        color=0x2f3136)
  ...
  async def _coowner(self, context: Context, member: discord.Member) -> None:
    if data := getConfig(context.guild.id):
      ...
      hacker1 = discord.Embed(
        description=f"❗ | Co Owner role is not setuped in {context.guild.name}",
        color=0x2f3136)
      hacker1.set_author(name=f"{context.author}",
                         icon_url=f"{context.author.avatar}")
      hacker1.set_thumbnail(url=f"{context.author.avatar}")
  ...
  async def _admin(self, context: Context, member: discord.Member) -> None:
    if data := getConfig(context.guild.id):
      ...
      hacker1 = discord.Embed(
        description=f"❗ | Admin role is not setuped in {context.guild.name}",
        color=0x2f3136)
      hacker1.set_author(name=f"{context.author}",
                         icon_url=f"{context.author.avatar}")
      hacker1.set_thumbnail(url=f"{context.author.avatar}")
  ...
  async def _headmod(self, context: Context, member: discord.Member) -> None:
    if data := getConfig(context.guild.id):
      ...
      hacker1 = discord.Embed(
        description=f"❗ | Head Mod role is not setuped in {context.guild.name}",
        color=0x2f3136)
      hacker1.set_author(name=f"{context.author}",
                         icon_url=f"{context.author.avatar}")
      hacker1.set_thumbnail(url=f"{context.author.avatar}")
  ...
  async def _mod(self, context: Context, member: discord.Member) -> None:
    if data := getConfig(context.guild.id):
      ...
      hacker1 = discord.Embed(
        description=f"❗ | Mod role is not setuped in {context.guild.name}",
        color=0x2f3136)
      hacker1.set_author(name=f"{context.author}",
                         icon_url=f"{context.author.avatar}")
      hacker1.set_thumbnail(url=f"{context.author.avatar}")
  ...
  async def _girlmod(self, context: Context, member: discord.Member) -> None:
    if data := getConfig(context.guild.id):
      ...
      hacker1 = discord.Embed(
        description=f"❗ | Girl Mod role is not setuped in {context.guild.name}",
        color=0x2f3136)
      hacker1.set_author(name=f"{context.author}",
                         icon_url=f"{context.author.avatar}")
      hacker1.set_thumbnail(url=f"{context.author.avatar}")
