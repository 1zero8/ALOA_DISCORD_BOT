...
class View(discord.ui.View):
  ...
  def add_buttons(self):
    self.startB = Buttons(emoji="<:backward:1096422644020158546>",
                          style=discord.ButtonStyle.grey,
                          command=self.set_page,
                          args=0,
                          ctx=self.ctx)
    self.backB = Buttons(emoji="<:back11:1096422625263222834>",
                         style=discord.ButtonStyle.grey,
                         command=self.to_page,
                         args=-1,
                         ctx=self.ctx)
    self.nextB = Buttons(emoji="<:forward1:1096422544283799582>",
                         style=discord.ButtonStyle.grey,
                         command=self.to_page,
                         args=+1,
                         ctx=self.ctx)
    self.endB = Buttons(emoji="<:forward:1096422570124918804>",
                        style=discord.ButtonStyle.grey,
                        command=self.set_page,
                        args=len(self.options) - 1,
                        ctx=self.ctx)
    self.quitB = Buttons(emoji="<:Home:1096422326125482075>",
                         style=discord.ButtonStyle.grey,
                         command=self.quit,
                         ctx=self.ctx)
  ...
  def gen_embeds(self):
    options, embeds = [], []
    options.append(
      discord.SelectOption(label="Home", emoji='<:Home:1096422326125482075>'))
    ...
    for cog in self.get_cogs():
      if "help_custom" in dir(cog):
        ...
        embed = discord.Embed(title=f"{emoji} {label} Commands",
                              color=0x2f3136)
        embed.set_footer(
          text="Made with ❤ By ~ Victorious Clasher#1430",
          icon_url=
          "https://media.discordapp.net/attachments/1070002726844514324/1077953428459888650/9879d6ae061333c7c3345ef01925a610.jpg?width=410&height=406"
        )
